#!/usr/bin/env python3
"""
barber context sentinel — a UserPromptSubmit hook for Claude Code.
...
exceeds BARBER_CTX_TOKENS *and* that has been idle longer than BARBER_IDLE_S,
it intervenes, in one of two modes (BARBER_MODE):
...
Config (env, all optional):
  BARBER_CTX_TOKENS  resident-context threshold (default 150000)
  BARBER_IDLE_S      idle threshold in seconds  (default 3600)
  BARBER_MODE        advise | block | off       (default advise)

Install (~/.claude/settings.json):
  "hooks": { "UserPromptSubmit": [ { "hooks": [ { "type": "command",
    "command": "python3 /path/to/barber/hooks/context_sentinel.py" } ] } ] }
...
CTX_THRESHOLD = int(os.environ.get("BARBER_CTX_TOKENS", "150000"))
IDLE_S = int(os.environ.get("BARBER_IDLE_S", "3600"))
MODE = os.environ.get("BARBER_MODE", "advise").lower()
...
def main():
    ...
    if MODE == "block":
        print(json.dumps({
            "decision": "block",
            "reason": (
                f"[barber] This session holds ~{ctx_k}K tokens of context and has been idle "
                ...
                f"(Disable with BARBER_MODE=off)"
            ),
        }))
        return
    ...
    print(json.dumps({
        "hookSpecificOutput": {
            ...
            "additionalContext": (
                f"[barber] Resident context is ~{ctx_k}K tokens and this session was "
